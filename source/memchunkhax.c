...
static void gspwn_copy(void *dest, void *src, uint32_t length,
                       int check, int check_offset, uintptr_t check_mem,
                       const struct app_offsets *app)
{
    ...
    while(*check_loc != check) {
        ...
        uint32_t arr2[] = {4, (uint32_t)dest, check_mem, check_offset + 4,
                           0xFFFFFFFF, 0xFFFFFFFF, 8, 0};
...
{
    register const void *r0 __asm__("r0");
    register const void *r4 __asm__("r4");

    // I need some memory locations to use
    uint32_t *mem_hax_mem = (uint32_t *)(linear + 0x50000);
    ...
    uint32_t *arm11_buffer = (uint32_t *)(linear + 0x2000);
    arm11_buffer[0] = 1;
    ...
    gspwn_copy(mem_hax_mem_free, arm11_buffer, 0x10,
               fw->kernel_patch_address, 4, linear + 0x1000, app);
    ...
    while (1) {
        ...
        r0 = func;
        __asm__ volatile ("svc 8" : "+r"(r0) : "r"(r4) : "r1");
    }
}
